...
export default function AdminAbonnementsListPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      snap.forEach((d) => {
        const data = d.data();
        if (isHiddenSystemAbonnement(data as Record<string, unknown>)) return;
  ...
  async function handleDelete(id: string) {
    try {
      await deleteDoc(doc(getFirebaseFirestore(), ABONNEMENTS_COLLECTION, id));
    ...
    <div className="mx-auto max-w-7xl">
      <div className="mb-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-xl text-slate-600">
            Fiches stockées dans Firestore — même source que le site public et
            l&apos;application.
          </p>
        </div>
        <Link
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white/60 px-6 py-16 text-center">
          <p className="text-slate-600">Aucun abonnement dans Firestore.</p>
          <Link
            href="/admin/abonnements/nouveau"
        ...
        <ul className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
          {items.map(({ id, data }) => (
            <li key={id}>
              <AbonnementCard id={id} data={data} onDelete={handleDelete} />
            </li>
          ))}
        </ul>
      )}
